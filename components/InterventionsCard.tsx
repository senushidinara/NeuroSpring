...
const InterventionsCard: React.FC<InterventionsCardProps> = ({ data, manualInterventions = [] }) => {
    const simulatorInterventions = data?.interventions || [];
    
    const combined = [...manualInterventions, ...simulatorInterventions];
    const uniqueInterventions = Array.from(new Set(combined));
    ...
    return (
        <div className="bg-slate-800 rounded-lg p-4 shadow-lg h-full transition-transform duration-200 hover:scale-[1.02] cursor-pointer">
            ...
            {!data && manualInterventions.length === 0 ? (
                ...
                <ul className="space-y-2">
                    {uniqueInterventions.map((item, index) => {
                        const isManual = manualInterventions.includes(item);
                        return (
                             <li key={`${item}-${index}`} className={`text-slate-300 text-sm p-2 rounded transition-all duration-300 ${isManual ? 'bg-cyan-900/50 border-l-2 border-cyan-400 font-semibold' : 'bg-slate-700/50'}`}>
                                {item}
                            </li>
                        );
                    })}
